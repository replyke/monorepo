...
<ParamField path="file" type="UniversalFile" required>
  The file to be uploaded. Can be a browser `File` or a React Native `{ uri, name, type }` object.
</ParamField>

<ParamField path="pathParts" type="string[]" required>
  An array of strings representing the hierarchical path to store the file.
</ParamField>
...
The `uploadFile` function returns a promise resolving to an object of type `UploadResponse 
| void`. On successful upload, it returns an `UploadResponse` object:
