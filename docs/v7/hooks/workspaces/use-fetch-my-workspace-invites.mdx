...
`{ data: MyWorkspaceInvitation[] }` — each item a `MyWorkspaceInvitation`: a [WorkspaceInvitation](/data-models/workspace-invitation) with `workspace` and `invitedBy` embedded (see the note on that page). Never paginated.
