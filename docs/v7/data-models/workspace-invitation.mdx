...
<Note>
  **[`GET /me/workspace-invites`](/api-reference/workspaces/invitations/list-my-invites) returns an enriched variant of this shape.** Every other read of a `WorkspaceInvitation` (listing what a workspace has issued, creating one, resending one) is by a capability-holder who already has full reach on the workspace, so `workspaceId`/`invitedBy` as bare ids are enough. The my-invites read is different: it's addressed to the *invitee*, who otherwise has no reach on the workspace at all (a direct workspace read 404s for a non-member) or the inviter. So that one endpoint replaces `invitedBy` with a populated object and adds a sibling `workspace` field, so the invite can be rendered ("X invited you to *Y*") without a follow-up read per invitation:

  | Property | Type | Description |
  |----------|------|-------------|
  | `workspace` | `{ id: string, name: string }` | The target workspace, embedded. `workspaceId` is still present alongside it. |
  | `invitedBy` | `{ id: string, name: string \| null, username: string \| null, avatar: string \| null, reputation: number }` | The inviter, embedded in place of the bare id. |
</Note>
