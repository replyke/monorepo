 ...
 * now`). Requires the `invite` capability (or owner). Always returned in full —
 * this list is deliberately unpaginated.
 *
 * This is the OUTBOX read — invites this workspace ISSUED. For the invites
 * addressed TO the caller, use `fetchMyWorkspaceInvites`.
 */
export async function fetchWorkspaceInvites(
  ...
  const response =
    await client.projectInstance.get<FetchWorkspaceInvitesResponse>(
