...
/**
 * The four conversation-mute duration choices. Send the CHOICE, never a raw
 * timestamp — the server resolves it and represents "forever" via the explicit
 * `mutedForever` signal on the returned member. Mirrors the server's
 * `MUTE_DURATIONS` (server `src/helpers/push/muteDuration.ts`).
 */
