  ...
  const removeAccount = useCallback(
    async ({ userId }: { userId: string }) => {
      ...
      try {
        ...
        if (deviceIdentifier) payload.device = deviceIdentifier;
        ...
        try {
          ...
          //
          // With a `device` in the request the atomic guarantee is live: the
          // server either removed the push binding and the token family
          ...
          //
          // With NO device there is no unbind, so there is nothing for the
          // guarantee to protect, and the old best-effort behaviour is kept
          // deliberately. Broadening strictness here would be a real
          // regression rather than a hardening: `/auth/sign-out` answers 204
          // for every write and token failure when no device is sent, so the
          // only thing left that can fail is the TRANSPORT — which would mean
